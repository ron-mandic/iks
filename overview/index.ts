...
class Slider {
  ...
  onMove(e) {
    ...
    let scrollX =
      ...
      window.innerWidth / 4;
    if (scrollX / innerWidth >= 0 && scrollX / innerWidth <= 0.33) {
      if (store.name !== 'Doaa') {
        store.name = 'Doaa';
        heading.innerHTML = store.name;
        anchor.setAttribute('data-suffix', store.name);
        anchor.classList.remove('disabled');
      }
    } else if (scrollX / innerWidth > 0.33 && scrollX / innerWidth <= 0.66) {
      if (store.name !== 'Lava') {
        store.name = 'Lava';
        heading.innerHTML = store.name;
        anchor.setAttribute('data-suffix', store.name);
    ...
    } else if (scrollX / innerWidth > 0.66 && scrollX / innerWidth <= 1) {
      if (store.name !== 'Maya') {
        store.name = 'Maya';
        heading.innerHTML = store.name;
        anchor.setAttribute('data-suffix', store.name);
    ...
    console.log(scrollX, store.name);
  }
}
...
class Plane extends GlObject {
  init(el) {
    ...
    this._texture = loader.load(this.img.src, (_texture) => {
      _texture.minFilter = THREE.LinearFilter;
      _texture.generateMipmaps = false;

      this.mat.uniforms.uTexture.value = _texture;
